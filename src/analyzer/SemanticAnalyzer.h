...
private:
    ...
    void visit(const StateNode& node, std::shared_ptr<SymbolTable> table);
    void visit(const EventHandlerNode& node, const std::string& component_type);
    ...
    std::unordered_set<std::string> valid_events_;
